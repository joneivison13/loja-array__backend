...
export default {
    ...
    async create(req:Request, res:Response){
        ...
        await database('products_has_user').insert({
            products_idproducts,
    ...
    async remove(req:Request, res:Response){
        ...
        const productQtd = await database('products_has_user').select('products_has_user_qtd').where({products_idproducts, user_iduser:req.iduser})
        ...
        if(Number(qtd) > Number(productQtd[0].products_has_user_qtd)){
          await database('products_has_user').delete('*').where({user_iduser:req.iduser, products_idproducts})
          return res.json({message:'Produto apagado com sucesso'})
        }

        await database('products_has_user').update({products_has_user_qtd:productQtd[0].products_has_user_qtd - Number(qtd)}).where({user_iduser:req.iduser, products_idproducts})
